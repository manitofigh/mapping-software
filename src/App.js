...
function Geocoding() {
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      try {
        const response = await fetch(
          `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(address)}&key=AIzaSyBIEXaswR68t2n5B34h_FEtp585wbVnGIQ`,
        );
        ...
        if (data.status === "OK") {
          ...
          const geocodedResult = {
            ...
            postcode: result.address_components.find((component) =>
  ...
  const dragEnter = (ev) => {
    ...
    if (ev.target.id == "drop_zone") ev.target.classList.add("dragover");
  };
  const dragLeave = (ev) => {
    // console.log("dragLeave");
    if (ev.target.id == "drop_zone") ev.target.classList.remove("dragover");
  };
    ...
    <div>
      ...
      >
        ...
        <form>
          ...
          <label id="upload_label" htmlFor="actual_button">
            Upload File
          </label>
        </form>
      </div>
